...
static CUresult cycle_reserved_address(CUdeviceptr d_mem, ssize_t size) {
  // ROCm does not return physical VRAM to mem_get_info while the old virtual
  // reservation is held. Sleep mode still needs the same tensor pointer to
  // remain reserved for wake_up, so release and immediately re-reserve the VA.
