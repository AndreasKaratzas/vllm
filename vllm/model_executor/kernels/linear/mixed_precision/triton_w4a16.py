    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        if self.w_zp_name is not None:
            ...
            if zp is not None:
                ...
                if zp.data.shape == expected_shape:
                    qzeros = zp.data.contiguous()
                else:
                    assert zp.data.shape == transposed_shape, (
                        "qzeros shape mismatch before TritonW4A16 "
                        f"post-processing: {zp.data.shape}; expected "
                        f"{expected_shape} or {transposed_shape}"
                    )
