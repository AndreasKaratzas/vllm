...
@pytest.mark.parametrize("decode_threshold", [1, 2])
def test_split_decodes_prefills_and_extends_decode_batch_without_seq_lens_upper_bound(
    decode_threshold,
):
    common_metadata = create_common_attn_metadata(
        BatchSpec(seq_lens=[32, 40, 48], query_lens=[1, 1, 1]),
        block_size=16,
        device=torch.device("cpu"),
    ).replace(seq_lens_cpu_upper_bound=None)

    assert split_decodes_prefills_and_extends(common_metadata, decode_threshold) == (
        3,
        0,
        0,
        3,
