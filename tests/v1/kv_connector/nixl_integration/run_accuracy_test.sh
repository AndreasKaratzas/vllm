...
init_visible_device_list() {
  local visible_devices="${CUDA_VISIBLE_DEVICES:-${HIP_VISIBLE_DEVICES:-}}"
...
select_visible_devices() {
  local start_idx=$1
  local count=$2
  local total=${#VISIBLE_DEVICE_LIST[@]}
  local selected=()

  if (( count > total )); then
    echo "Requested ${count} GPUs, but only ${total} visible: ${VISIBLE_DEVICE_LIST[*]}" >&2
    return 1
  fi

  for ((idx = 0; idx < count; idx++)); do
    selected+=("${VISIBLE_DEVICE_LIST[$(((start_idx + idx) % total))]}")
  done
...
run_tests_for_model() {
  local model_name=$1
  local total_gpus_needed=$(( \
    (NUM_PREFILL_INSTANCES * PREFILLER_TP_SIZE) \
    + (NUM_DECODE_INSTANCES * DECODER_TP_SIZE) \
  ))

  if (( ${#VISIBLE_DEVICE_LIST[@]} < total_gpus_needed )); then
    echo "FAIL: ${model_name} needs ${total_gpus_needed} GPUs, " \
      "but only ${#VISIBLE_DEVICE_LIST[@]} are visible: ${VISIBLE_DEVICE_LIST[*]}"
    return 1
  fi
  ...
  for i in $(seq 0 $((NUM_PREFILL_INSTANCES-1))); do
    # Calculate GPU IDs from the devices visible to this process.
    GPU_ID=$(select_visible_devices "$NEXT_DEVICE_SLOT" "$PREFILLER_TP_SIZE")
    NEXT_DEVICE_SLOT=$(((NEXT_DEVICE_SLOT + PREFILLER_TP_SIZE) % ${#VISIBLE_DEVICE_LIST[@]}))
  ...
  for i in $(seq 0 $((NUM_DECODE_INSTANCES-1))); do
    # Calculate GPU IDs from the devices visible to this process, continuing
    # after the prefill allocations to avoid overlap.
    GPU_ID=$(select_visible_devices "$NEXT_DEVICE_SLOT" "$DECODER_TP_SIZE")
    NEXT_DEVICE_SLOT=$(((NEXT_DEVICE_SLOT + DECODER_TP_SIZE) % ${#VISIBLE_DEVICE_LIST[@]}))
    # Calculate port number (base port + instance number)
    PORT=$((8200 + i))
...
init_visible_device_list
echo "Visible GPU list for this run: ${VISIBLE_DEVICE_LIST[*]}"
