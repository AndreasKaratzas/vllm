...
def test_gpt_oss_attention_quantization(
    ...
    if "amd/gpt-oss-20b-MoE-Quant-W-MXFP4-A-FP8-KV-FP8" in model_name:
        if current_platform.is_rocm() and not on_gfx950():
            pytest.skip("MXFP4 MoE backend is supported on ROCm gfx950 only")
        if on_gfx950():
